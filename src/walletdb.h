...
public:
    ...
    bool WriteZGIFSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZGIFSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZGIFSeed_deprecated(uint256& seed);
    bool EraseZGIFSeed();
    bool EraseZGIFSeed_deprecated();

    bool WriteZGIFCount(const uint32_t& nCount);
    bool ReadZGIFCount(uint32_t& nCount);
