...
CzGIFWallet::CzGIFWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zgif seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZGIFSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZGIFSeed_deprecated()) {
                    LogPrintf("%s: Updated zGIF seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zgif seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zgif wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzGIFWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZGIFCount(nCountLastUsed);
    else if (!walletdb.ReadZGIFCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzGIFWallet::Lock()
{
    seedMaster = 0;
}

void CzGIFWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzGIFWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZGIF(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzGIFWallet::LoadMintPoolFromDB()
{
...
void CzGIFWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzGIFWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzGIFWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zgifTracker->HasPubcoinHash(pMint.first)) {
...
bool CzGIFWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZGIF(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zgifTracker which also adds to database
    pwalletMain->zgifTracker->Add(dMint, true);
...
void CzGIFWallet::SeedToZGIF(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzGIFWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzGIFWallet::GenerateDeterministicZGIF(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzGIFWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzGIFWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
