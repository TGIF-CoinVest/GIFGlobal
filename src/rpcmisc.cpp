...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total tgifglobal balance of the wallet (excluding zerocoins)\n"
            ...
            "  \"zGIFsupply\" :\n"
            "  {\n"
            "     \"1\" : n,            (numeric) supply of 1 zGIF denomination\n"
            "     \"5\" : n,            (numeric) supply of 5 zGIF denomination\n"
            "     \"10\" : n,           (numeric) supply of 10 zGIF denomination\n"
            "     \"50\" : n,           (numeric) supply of 50 zGIF denomination\n"
            "     \"100\" : n,          (numeric) supply of 100 zGIF denomination\n"
            "     \"500\" : n,          (numeric) supply of 500 zGIF denomination\n"
            "     \"1000\" : n,         (numeric) supply of 1000 zGIF denomination\n"
            "     \"5000\" : n,         (numeric) supply of 5000 zGIF denomination\n"
            "     \"total\" : n,        (numeric) The total supply of all zGIF denominations\n"
            "  }\n"
            "  \"keypoololdest\": xxxxxx,    (numeric) the timestamp (seconds since GMT epoch) of the oldest pre-generated key in the key pool\n"
            "  \"keypoolsize\": xxxx,        (numeric) how many new keys are pre-generated\n"
            "  \"unlocked_until\": ttt,      (numeric) the timestamp in seconds since epoch (midnight Jan 1 1970 GMT) that the wallet is unlocked for transfers, or 0 if the wallet is locked\n"
            "  \"paytxfee\": x.xxxx,         (numeric) the transaction fee set in tgifglobal/kb\n"
            "  \"relayfee\": x.xxxx,         (numeric) minimum relay fee for non-free transactions in tgifglobal/kb\n"
    ...
    UniValue zgifObj(UniValue::VOBJ);
    for (auto denom : libzerocoin::zerocoinDenomList) {
        zgifObj.push_back(Pair(to_string(denom), ValueFromAmount(chainActive.Tip()->mapZerocoinSupply.at(denom) * (denom*COIN))));
    }
    zgifObj.push_back(Pair("total", ValueFromAmount(chainActive.Tip()->GetZerocoinSupply())));
    obj.push_back(Pair("zGIFsupply", zgifObj));
...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"tgifglobaladdress\"\n"
            "\nReturn information about the given tgifglobal address.\n"

            "\nArguments:\n"
            "1. \"tgifglobaladdress\"     (string, required) The tgifglobal address to validate\n"

            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"tgifglobaladdress\", (string) The tgifglobal address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: TGIFGLOBAL address and we have full public key:
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
        throw runtime_error(
            ...
            "2. \"keys\"       (string, required) A json array of keys which are tgifglobal addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) tgifglobal address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"tgifglobaladdress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"

            "\nArguments:\n"
            "1. \"tgifglobaladdress\"  (string, required) The tgifglobal address to use for the signature.\n"
