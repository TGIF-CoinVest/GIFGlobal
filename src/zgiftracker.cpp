...
CzGIFTracker::CzGIFTracker(std::string strWalletFile)
{
...
CzGIFTracker::~CzGIFTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzGIFTracker::Init()
{
...
bool CzGIFTracker::Archive(CMintMeta& meta)
{
...
bool CzGIFTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzGIFTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzGIFTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzGIFTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzGIFTracker::GetSerialHashes()
{
...
CAmount CzGIFTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzGIFTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzGIFTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzGIFTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzGIFTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzGIFTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzGIFTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzGIFTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzGIFTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzGIFTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzGIFTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzGIFTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzGIFTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzGIFTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzGIFTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzGIFTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzGIFTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzgif from DB\n", __func__, listDeterministicDB.size());
    }
