...
#ifndef TGIFGLOBAL_ZGIFWALLET_H
#define TGIFGLOBAL_ZGIFWALLET_H
...
public:
    CzGIFWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZGIF(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZGIF(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //TGIFGLOBAL_ZGIFWALLET_H
