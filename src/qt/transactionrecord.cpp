...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    if (wtx.IsCoinStake()) {
        ...
        if (wtx.IsZerocoinSpend() && (fZSpendFromMe || wallet->zgifTracker->HasMintTx(hash))) {
            //zGIF stake reward
            sub.involvesWatchAddress = false;
            sub.type = TransactionRecord::StakeZGIF;
        ...
        BOOST_FOREACH (const CTxOut& txout, wtx.vout) {
            ...
            if (mine) {
                ...
                if (ExtractDestination(txout.scriptPubKey, address) && IsMine(*wallet, address)) {
                    // Received by TGIFGLOBAL Address
...
bool IsZGIFType(TransactionRecord::Type type)
{
    switch (type) {
        case TransactionRecord::StakeZGIF:
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    else if (type == TransactionRecord::Generated || type == TransactionRecord::StakeMint || type == TransactionRecord::StakeZGIF || type == TransactionRecord::MNReward) {
