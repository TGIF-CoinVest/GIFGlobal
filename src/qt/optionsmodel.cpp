...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::GIF);
    ...
    if (!settings.contains("nAnonymizeTGIFGLOBALAmount"))
        settings.setValue("nAnonymizeTGIFGLOBALAmount", 1000);

    nAnonymizeTGIFGLOBALAmount = settings.value("nAnonymizeTGIFGLOBALAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeTGIFGLOBALAmount"))
        SoftSetArg("-anonymizetgifglobalamount", settings.value("nAnonymizeTGIFGLOBALAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in tgifglobal.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeTGIFGLOBALAmount:
            return QVariant(nAnonymizeTGIFGLOBALAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeTGIFGLOBALAmount:
            nAnonymizeTGIFGLOBALAmount = value.toInt();
            settings.setValue("nAnonymizeTGIFGLOBALAmount", nAnonymizeTGIFGLOBALAmount);
            emit anonymizeTGIFGLOBALAmountChanged(nAnonymizeTGIFGLOBALAmount);
            break;
        case CoinControlFeatures:
