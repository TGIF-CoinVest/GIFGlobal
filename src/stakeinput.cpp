...
// The zGIF block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zGIF is a hash of the serial
...
{
    //Create an output returning the zGIF that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZGIFOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zGIF output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zGIF", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZGIFOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zGIF output", __func__);
...
{
    CzGIFTracker* zgifTracker = pwallet->zgifTracker.get();
    CMintMeta meta;
    if (!zgifTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zgifTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a GIF stake is the outpoint
