...
pkgdatadir = $(datadir)/tgifglobal
pkgincludedir = $(includedir)/tgifglobal
pkglibdir = $(libdir)/tgifglobal
pkglibexecdir = $(libexecdir)/tgifglobal
...
CONFIG_HEADER = $(top_builddir)/src/config/tgifglobal-config.h
...
am__DIST_COMMON = $(srcdir)/Makefile.in \
	...
	$(top_srcdir)/src/config/tgifglobal-config.h.in \
...
ACLOCAL = ${SHELL} /tmp/TGIFGLOBAL/build-aux/missing aclocal-1.15
AMTAR = $${TAR-tar}
AM_DEFAULT_VERBOSITY = 0
AR = /usr/bin/ar
ARFLAGS = cr
AUTOCONF = ${SHELL} /tmp/TGIFGLOBAL/build-aux/missing autoconf
AUTOHEADER = ${SHELL} /tmp/TGIFGLOBAL/build-aux/missing autoheader
AUTOMAKE = ${SHELL} /tmp/TGIFGLOBAL/build-aux/missing automake-1.15
AWK = gawk
BDB_CFLAGS = 
BDB_CPPFLAGS = 
BDB_LIBS = -ldb_cxx-4.8
BITCOIN_CLI_NAME = tgifglobal-cli
BITCOIN_DAEMON_NAME = tgifglobald
BITCOIN_GUI_NAME = tgifglobal-qt
BITCOIN_TX_NAME = tgifglobal-tx
...
MAINT = 
MAKEINFO = ${SHELL} /tmp/TGIFGLOBAL/build-aux/missing makeinfo
MAKENSIS = 
MANIFEST_TOOL = :
MINIUPNPC_CPPFLAGS = 
...
OTOOL64 = 
PACKAGE = tgifglobal
PACKAGE_BUGREPORT = www.tgifglobal.org
PACKAGE_NAME = TGIFGLOBAL Core
PACKAGE_STRING = TGIFGLOBAL Core 1.0.0
PACKAGE_TARNAME = tgifglobal
PACKAGE_URL = 
...
ZMQ_CFLAGS = 
ZMQ_LIBS = -lzmq
abs_builddir = /tmp/TGIFGLOBAL
abs_srcdir = /tmp/TGIFGLOBAL
abs_top_builddir = /tmp/TGIFGLOBAL
abs_top_srcdir = /tmp/TGIFGLOBAL
ac_ct_AR = ar
ac_ct_CC = gcc
ac_ct_CXX = g++
ac_ct_DUMPBIN = 
...
install_sh = ${SHELL} /tmp/TGIFGLOBAL/build-aux/install-sh
...
COVERAGE_INFO = baseline_filtered_combined.info baseline.info block_test.info \
  leveldb_baseline.info test_tgifglobal_filtered.info total_coverage.info \
  baseline_filtered.info block_test_filtered.info \
  leveldb_baseline_filtered.info test_tgifglobal_coverage.info test_tgifglobal.info

OSX_APP_BUILT = $(OSX_APP)/Contents/PkgInfo $(OSX_APP)/Contents/Resources/empty.lproj \
  $(OSX_APP)/Contents/Resources/bitcoin.icns $(OSX_APP)/Contents/Info.plist \
  $(OSX_APP)/Contents/MacOS/TGIFGLOBAL-Qt $(OSX_APP)/Contents/Resources/Base.lproj/InfoPlist.strings
...
src/config/tgifglobal-config.h: src/config/stamp-h1
	@test -f $@ || rm -f src/config/stamp-h1
	@test -f $@ || $(MAKE) $(AM_MAKEFLAGS) src/config/stamp-h1

src/config/stamp-h1: $(top_srcdir)/src/config/tgifglobal-config.h.in $(top_builddir)/config.status
	@rm -f src/config/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status src/config/tgifglobal-config.h
$(top_srcdir)/src/config/tgifglobal-config.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f src/config/stamp-h1
	touch $@

distclean-hdr:
	-rm -f src/config/tgifglobal-config.h src/config/stamp-h1
...
$(OSX_APP)/Contents/MacOS/TGIFGLOBAL-Qt: $(BITCOIN_QT_BIN)
	$(MKDIR_P) $(@D)
...
$(APP_DIST_EXTRAS): $(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/TGIFGLOBAL-Qt
...
$(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/TGIFGLOBAL-Qt: $(OSX_APP_BUILT) $(OSX_PACKAGING)
...
#test_tgifglobal.info: baseline_filtered_combined.info
#	$(MAKE) -C src/ check
#	$(LCOV) -c -d $(abs_builddir)/src -t test_tgifglobal -o $@
#	$(LCOV) -z -d $(abs_builddir)/src
#	$(LCOV) -z -d $(abs_builddir)/src/leveldb

#test_tgifglobal_filtered.info: test_tgifglobal.info
#	$(LCOV) -r $< "/usr/include/*" -o $@

#block_test.info: test_tgifglobal_filtered.info
#	$(MKDIR_P) qa/tmp
...
#test_tgifglobal_coverage.info: baseline_filtered_combined.info test_tgifglobal_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_tgifglobal_filtered.info -o $@

#total_coverage.info:  baseline_filtered_combined.info test_tgifglobal_filtered.info block_test_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_tgifglobal_filtered.info -a block_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#test_tgifglobal.coverage/.dirstamp:  test_tgifglobal_coverage.info
#	$(GENHTML) -s $< -o $(@D)
#	@touch $@
...
#cov: test_tgifglobal.coverage/.dirstamp total.coverage/.dirstamp

.INTERMEDIATE: $(COVERAGE_INFO)

clean-local:
	rm -rf test_tgifglobal.coverage/ total.coverage/ $(OSX_APP)
